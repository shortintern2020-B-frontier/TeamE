  ...
  <v-row id="flex-container">
    <v-col cols="3">
      ...
      <v-card
        ...
        :src="region.src"
        flat
        @click="triggerEvent(region.region)"
      >
        <v-img
          :src="region.src"
          class="white--text align-end"
    ...
    <v-col cols="8">
      ...
      <div v-else-if="Object.keys(filter).length">
        <span class="packs font-weight-bold text-h4 pl-6">{{
          region_choice
        }}</span>
        <v-divider class="mb-2" id="divider-top"></v-divider>
        <v-card
          class="pb-2"
          v-for="(regionitem, index) in filter"
          :key="regionitem.package_name"
        ...
        >
          <router-link :to="{ name: 'Detail', params: { id: index } }">
            ...
            >
              <v-card-title
                class="white--text align-end"
                v-text="regionitem.package_name"
              ></v-card-title>
            </v-img>
